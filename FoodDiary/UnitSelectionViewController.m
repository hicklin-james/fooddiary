...
//  FoodDiary
//
//  Created by James Hicklin on 2013-07-02.
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  if (indexPath.row == 0) {
    if ([profile boolForKey:@"unitType"] == NO) {
      cell.accessoryType = UITableViewCellAccessoryCheckmark;
    }
    cell.textLabel.text = @"English (inches, lbs)";
    cell.textLabel.font = [UIFont fontWithName:@"Helvetica" size:12];
  }
  
  else {
    if ([profile boolForKey:@"unitType"] == YES) {
      cell.accessoryType = UITableViewCellAccessoryCheckmark;
    }
    cell.textLabel.text = @"Metric (cm, kgs)";
    cell.textLabel.font = [UIFont fontWithName:@"Helvetica" size:12];
  }
